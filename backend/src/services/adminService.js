 ...
 */
const { User, License, Device, Assignment, AuditLog, AppSetting } = require('../models');
const crypto = require('../utils/crypto');
const { buildLicenseResponse, buildDeviceResponse, clearLicenseCache } = require('./licenseService');
...
function createLicense(data, adminId) {
  ...
  if (data.durationDays) {
    ...
    endAt = new Date(start.getTime() + data.durationDays * 86400000).toISOString();
  } else if (data.durationHours) {
    const start = new Date();
    endAt = new Date(start.getTime() + data.durationHours * 3600000).toISOString();
  } else if (data.endAt) {
    endAt = data.endAt;
  }
  ...
  if (data.isPermanent) {
    endAt = new Date('2099-12-31').toISOString();
  }
...
function clearLicenseCacheAll(licenseId) {
  // In production, track which devices have which license cached
}
