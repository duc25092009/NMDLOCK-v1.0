...
/**
 * Returns current datetime in SQLite-compatible format (YYYY-MM-DD HH:MM:SS).
 * Matches SQLite's datetime('now') output.
 */
function nowSQL() {
  const d = new Date();
...
/**
 * Converts a Date to SQLite-compatible format.
 */
function toSQL(date) {
...
const License = {
  ...
  extend: (id, additionalDays) => {
    ...
    return getDb().prepare('UPDATE licenses SET end_at = ?, updated_at = datetime(\'now\') WHERE id = ?').run(toSQL(newEnd), id);
  },
...
const AuditLog = {
  ...
  getRecentFailures: (minutes = 5) => {
    const since = toSQL(new Date(Date.now() - minutes * 60000));
