...
class ThermalMonitor @Inject constructor(
     ...
     * Parses thermal zone temp files
     */
        ...
        try {
            ...
            zoneTempsResult.lines().forEach { line ->
                ...
                if (parts.size >= 2) {
                    ...
                    // Use local copies to avoid smart cast issues with captured vars in lambda
                    val currentCpu = cpuTemp
                    val currentGpu = gpuTemp
                    when {
                        zoneType.contains("cpu") || zoneType.contains("cpu-") -> {
                            if (currentCpu == null || tempC > currentCpu) cpuTemp = tempC
                        }
                        zoneType.contains("gpu") || zoneType.contains("gpu-") -> {
                            if (currentGpu == null || tempC > currentGpu) gpuTemp = tempC
                        }
                    }
